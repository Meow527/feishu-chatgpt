...
func (gpt ChatGPT) doAPIRequestWithRetry(url, method string, bodyType requestBodyType, requestBody interface{}, responseBody interface{}, client *http.Client, maxRetries int) error {
	...
	for retry = 0; retry <= maxRetries; retry++ {
		fmt.Println("重试次数：第", retry+1, "次")
		response, err = client.Do(req)
		fmt.Println("请求没有走vpn,所以超时了,返回的结果是nil")
		fmt.Println("--------------------")
...
func (gpt ChatGPT) sendRequestWithBodyType(link, method string, bodyType requestBodyType, requestBody interface{}, responseBody interface{}) error {
	var err error
	client := &http.Client{Timeout: 110 * time.Second}
	if gpt.HttpProxy == "" {
		err = gpt.doAPIRequestWithRetry(link, method, bodyType, requestBody, responseBody, client, 3)
	} else {
		...
		err = gpt.doAPIRequestWithRetry(link, method, bodyType, requestBody, responseBody, proxyClient, 3)
	}
